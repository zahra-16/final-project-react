...
export const TvDetail = () => {
  ...
  const handleRating = (rating) => {
    ...
    let ratedShows = JSON.parse(localStorage.getItem("ratedShows")) || [];
    const showExists = ratedShows.some((show) => show.id === detail.id);

    if (!showExists) {
      ratedShows.push({ ...detail, userRating: rating });
    } else {
      ratedShows = ratedShows.map((show) =>
        show.id === detail.id ? { ...show, userRating: rating } : show
      );
    }

    localStorage.setItem("ratedShows", JSON.stringify(ratedShows));
